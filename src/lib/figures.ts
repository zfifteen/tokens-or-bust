export type Tag = "hypothesis" | "measured" | "mixed" | "unscored";
export type Chapter =
  | "identity"
  | "prices"
  | "paper"
...
export const CHAPTERS: { id: Chapter; label: string; kicker: string }[] = [
  { id: "identity", label: "Identity", kicker: "The identity · Hypothesis" },
  { id: "prices", label: "Prices", kicker: "What silicon can charge · Measured" },
  { id: "paper", label: "Paper", kicker: "Paper on the books" },
  { id: "cash", label: "Cash", kicker: "Cash against capex" },
  { id: "tape", label: "Tape", kicker: "Index and credit" },
  { id: "hub", label: "Hub", kicker: "The hub" },
];
...
export function nextFigure(id: string) {
  const i = FIGURES.findIndex((f) => f.id === id);
  if (i < 0) return undefined;
  return FIGURES[(i + 1) % FIGURES.length];
}

export function prevFigure(id: string) {
  const i = FIGURES.findIndex((f) => f.id === id);
  if (i < 0) return undefined;
